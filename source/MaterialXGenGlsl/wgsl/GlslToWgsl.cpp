...
{
    // Strip carriage returns/newlines as well as spaces/tabs: library sources may use
    // CRLF (notably in the WASM preloaded library tree), and `std::getline` on '\n'
    // leaves a trailing '\r' that would otherwise defeat exact comparisons like
    // `trim(line) == "}"` (which would leave struct/overload scanning desynced).
...
bool isKnownReturnType(const string& t)
{
    if (t == "void" || t == "float" || t == "int" || t == "uint" || t == "bool")
        return true;
    if (t.compare(0, 3, "vec") == 0 || t.compare(0, 4, "ivec") == 0 ||
        t.compare(0, 4, "uvec") == 0 || t.compare(0, 3, "mat") == 0)
        return true;
    // MaterialX closure/shader struct types may be returned by library functions.
    if (t == "ClosureData" || t == "FresnelData" || t == "BSDF" || t == "EDF" ||
        t == "VDF" || t == "surfaceshader" || t == "volumeshader" ||
        t == "displacementshader" || t == "lightshader" || t == "material")
        return true;
    return false;
}
...
// ─── Consolidated GLSL→WGSL type map ────────────────────────────────────────
...
const size_t TYPE_MAP_SIZE = sizeof(TYPE_MAP) / sizeof(TYPE_MAP[0]);
} // namespace

string mapType(const string& glslType)
{
    // Combined GLSL sampler type used in function signatures.
    if (glslType == "texture2D")
        return "texture_2d<f32>";
    for (size_t i = 0; i < TYPE_MAP_SIZE; ++i)
    {
        if (glslType == TYPE_MAP[i].glsl)
            return TYPE_MAP[i].wgsl;
    }
    return glslType;
}
...
// ─── Stateless single-line rewrites ─────────────────────────────────────────

namespace
{

// Replace GLSL-style scalar casts with WGSL spellings (float() -> f32(), int() -> i32(), …).
string rewriteScalarCast(string line, const char* glslType, const char* wgslType)
{
    const size_t glslLen = std::strlen(glslType);
    const size_t wgslLen = std::strlen(wgslType);
    size_t pos = 0;
    ...
    while ((pos = line.find(token, pos)) != string::npos)
    {
        ...
        line.replace(pos, glslLen + 1, string(wgslType) + "(");
        pos += wgslLen + 1;
    }
    return line;
}

string rewriteFloatCast(string line)
{
    return rewriteScalarCast(std::move(line), "float", "f32");
}

string rewriteIntCast(string line)
{
    line = rewriteScalarCast(std::move(line), "int", "i32");
    return rewriteScalarCast(std::move(line), "uint", "u32");
}
...
string rewriteConst(const string& line)
{
    ...
    while ((pos = result.find("const ", pos)) != string::npos)
    {
        ...
        while (nameStart < result.size() && isSpace(result[nameStart]))
            nameStart++;
        ...
        while (typeEnd < result.size() && isSpace(result[typeEnd]))
            typeEnd++;
...
bool replaceOneTernary(string& line)
{
    ...
    size_t qBefore = q;
    while (qBefore > 0 && isSpace(line[qBefore - 1]))
        qBefore--;

    size_t qAfter = q + 1;
    while (qAfter < line.size() && isSpace(line[qAfter]))
        qAfter++;
    ...
    while (condStart < qBefore && isSpace(line[condStart]))
        condStart++;
    ...
    while (tStart < colonPos && isSpace(line[tStart]))
        tStart++;
    size_t tEnd = colonPos;
    while (tEnd > tStart && isSpace(line[tEnd - 1]))
        tEnd--;
    string tVal = trim(line.substr(tStart, tEnd - tStart));

    size_t fStart = colonPos + 1;
    while (fStart < line.size() && isSpace(line[fStart]))
        fStart++;
...
string rewriteVariableDecl(string line)
{
    ...
    for (size_t i = 0; i < TYPE_MAP_SIZE; ++i)
    {
        size_t typeLen = std::strlen(TYPE_MAP[i].glsl);
        if (line.size() > lineStart + typeLen + 1 &&
            line.compare(lineStart, typeLen, TYPE_MAP[i].glsl) == 0 &&
            isSpace(line[lineStart + typeLen]))
        {
            size_t nameStart = lineStart + typeLen;
            while (nameStart < line.size() && isSpace(line[nameStart]))
                nameStart++;
            ...
            while (afterName < line.size() && isSpace(line[afterName]))
                afterName++;
            ...
            if (afterName < line.size() && line[afterName] == '=')
            {
                ...
                while (rs < line.size() && isSpace(line[rs]))
                    rs++;
                return leading + "var " + varName + ": " + string(TYPE_MAP[i].wgsl) + " = " + line.substr(rs);
            }
            if (afterName < line.size() && line[afterName] == ';')
            {
                return leading + "var " + varName + ": " + string(TYPE_MAP[i].wgsl) + ";";
            }
            break;
        }
    }
    ...
    static const char* WGSL_TYPES[] = {
        "vec2f", "vec3f", "vec4f", "mat2x2f", "mat3x3f", "mat4x4f", "f32", "i32", "u32"
    };
    for (const char* wtype : WGSL_TYPES)
    {
        const size_t typeLen = std::strlen(wtype);
        if (line.size() > lineStart + typeLen + 1 &&
            line.compare(lineStart, typeLen, wtype) == 0 &&
            isSpace(line[lineStart + typeLen]))
        {
            size_t nameStart = lineStart + typeLen;
            while (nameStart < line.size() && isSpace(line[nameStart]))
            ...
            if (afterName < line.size() && line[afterName] == '=')
            {
                ...
                while (rs < line.size() && isSpace(line[rs]))
                    rs++;
                return line.substr(0, lineStart) + "var " + varName + ": " + wtype + " = " + line.substr(rs);
            }
            if (afterName < line.size() && line[afterName] == ';')
            {
                return line.substr(0, lineStart) + "var " + varName + ": " + wtype + ";";
            }
            break;
        }
    }
...
string rewriteMatrixCtors(string line)
{
    ...
    static const M MS[] = { { "mat2(", "mat2x2f(" }, { "mat3(", "mat3x3f(" }, { "mat4(", "mat4x4f(" } };
    for (const M& m : MS)
    {
        const string from = m.from;
        size_t pos = 0;
        while ((pos = line.find(from, pos)) != string::npos)
        {
            ...
            line.replace(pos, from.size(), m.to);
            pos += std::strlen(m.to);
        }
    }
    return line;
...
string rewriteMathBuiltins(string line)
{
    ...
    for (const Alias& a : ALIASES)
    {
        ...
        while ((pos = line.find(from, pos)) != string::npos)
        {
            ...
            line.replace(pos, from.size(), a.to);
            pos += std::strlen(a.to);
        }
    }
    return line;
...
string rewriteSampleLightSource(string line)
{
    ...
    while (a3 < line.size() && isSpace(line[a3]))
        a3++;
...
string rewriteDefine(const string& line)
{
    ...
    // Drop genglsl math built-in aliases (`#define mx_cos cos`, `#define
    // mx_float_bits_to_int floatBitsToInt`, ...) — a single-identifier value. The
    // call sites that matter are rewritten to WGSL built-ins by rewriteMathBuiltins.
    if (name.compare(0, 3, "mx_") == 0 &&
        value.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_") == string::npos)
        return "";

    // Type-valued define -> alias. Recognise scalar/vector/matrix types and the
    // MaterialX closure struct names.
    const string mapped = mapType(value);
    const bool isKnownType = (mapped != value) || value == "surfaceshader" ||
                             value == "BSDF" || value == "EDF" || value == "VDF" || value == "material";
    const bool isIdentifier = value.find_first_not_of(
                                  "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_") == string::npos;
    if (isKnownType && isIdentifier)
        return "alias " + name + " = " + (value == "vec3" || mapped != value ? mapped : value) + ";";

    // Numeric / expression define -> const. Integer literal => i32, else f32.
...
string rewriteForLoopInit(string line)
{
    ...
    while ((pos = line.find("for", pos)) != string::npos)
    {
        ...
        size_t paren = pos + 3;
        while (paren < line.size() && isSpace(line[paren]))
            paren++;
...
bool looksLikeTypedDecl(const string& segIn)
{
    ...
    if (seg.empty())
        return false;
    size_t at = 0;
    while (at < seg.size() && isSpace(seg[at]))
        at++;
    for (size_t i = 0; i < TYPE_MAP_SIZE; ++i)
    {
        const size_t typeLen = std::strlen(TYPE_MAP[i].glsl);
        if (seg.size() >= at + typeLen && seg.compare(at, typeLen, TYPE_MAP[i].glsl) == 0 &&
            (at + typeLen >= seg.size() || isSpace(seg[at + typeLen])))
        {
            size_t nameStart = at + typeLen;
            while (nameStart < seg.size() && isSpace(seg[nameStart]))
                nameStart++;
            size_t nameEnd = nameStart;
            ...
            while (after < seg.size() && isSpace(seg[after]))
                after++;
...
string rewriteBracelessControlSameLine(string line)
{
    ...
    else if (t.compare(0, 4, "else") == 0 && (t.size() == 4 || isSpace(t[4])))
    {
        size_t after = 4;
        while (after < t.size() && isSpace(t[after]))
            after++;
    ...
    {
        const char* prefixes[] = { "if", "while", "for" };
        for (const char* pfx : prefixes)
        {
            const size_t plen = std::strlen(pfx);
            if (t.compare(0, plen, pfx) == 0 && (t.size() == plen || isSpace(t[plen]) || t[plen] == '('))
            {
                const size_t open = t.find('(');
                if (open != string::npos)
                {
    ...
    while (bodyStart < t.size() && isSpace(t[bodyStart]))
        bodyStart++;
...
{
    size_t ls = 0;
    while (ls < line.size() && isSpace(line[ls]))
        ls++;
    size_t typeEnd = ls;
    while (typeEnd < line.size() && (isalnum(static_cast<unsigned char>(line[typeEnd])) || line[typeEnd] == '_'))
        typeEnd++;
    if (typeEnd == ls || typeEnd >= line.size() || !isSpace(line[typeEnd]))
        return line;
    ...
    for (const string& n : splitTopLevel(body))
    {
        ...
        if (nm.empty() || nm.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_") != string::npos)
...
string rewriteIncDec(string line)
{
    ...
    for (size_t k = 0; k < 2; ++k)
    {
        ...
        {
            size_t nameEnd = pos;
            while (nameEnd > 0 && isSpace(line[nameEnd - 1]))
                nameEnd--;
...
// ─── Combined-sampler / texture-sampling rewrites ───────────────────────────
...
// ─── Function-signature rewriting (out params -> ptr<function,T>) ────────────
...
// Detect and rewrite a GLSL function-definition signature line. On success
// returns true, writes the WGSL signature to `result`, and appends any
// out-parameter names to `outNames`.
bool rewriteSignature(const string& line, string& result, std::vector<string>& outNames,
                      std::vector<std::pair<string, string>>& valueParams)
{
    ...
    const string head = trim(line.substr(indentEnd, paren - indentEnd));
    ...
    const string tail = trim(line.substr(close + 1));
    if (!tail.empty() && tail[0] != '{')
        return false;

    const string paramList = line.substr(paren + 1, close - paren - 1);
    std::vector<string> params = splitTopLevel(paramList);

    string wgslParams;
    for (const string& p : params)
    {
        const string wp = rewriteParam(p, outNames, valueParams);
        if (wp.empty())
            continue;
        if (!wgslParams.empty())
            wgslParams += ", ";
...
string undoDerefInCallArgs(string line, const std::vector<string>& names)
{
    for (const string& name : names)
    {
        ...
        {
            size_t before = pos;
            while (before > 0 && isSpace(line[before - 1]))
                before--;
...
bool looksLikeControlHeader(const string& lineIn)
{
    ...
    if (t.compare(0, 4, "else") == 0 && (t.size() == 4 || isSpace(t[4])))
    {
        size_t after = 4;
        while (after < t.size() && isSpace(t[after]))
            after++;
...
string rewriteBoolUniformCondition(string line)
{
    ...
    for (const char* kw : KEYWORDS)
    {
        ...
        for (size_t pos = 0; (pos = line.find(k, pos)) != string::npos;)
        {
            ...
            while (identStart < line.size() && isSpace(line[identStart]))
                identStart++;
            if (identStart + 1 < line.size() && line[identStart] == 'u' && line[identStart + 1] == '_')
            {
                ...
                while (afterIdent < line.size() && isSpace(line[afterIdent]))
                    afterIdent++;
...
string rewriteAll(string line)
{
    ...
    line = rewriteFloatCast(std::move(line));
    line = rewriteIntCast(std::move(line));
...
{
    _funcActive = true;
    _seenOpenBrace = false;
    _braceDepth = 0;
    _injected = false;
    _outParams = std::move(outNames);

    // Build the mutable `var` copies for value parameters.
    _paramInjections.clear();
    for (const auto& vp : valueParams)
        _paramInjections.push_back("    var " + vp.first + ": " + vp.second + " = " + vp.first + ARG_SUFFIX + ";");

    string out = sig;
    countBraces(sig, _braceDepth, _seenOpenBrace);
    if (_seenOpenBrace)
    {
        ...
        if (bpos != string::npos && !_paramInjections.empty())
        {
            string inj;
            for (const string& i : _paramInjections)
                inj += " " + trim(i);
            out.insert(bpos + 1, inj);
        }
        _injected = true;
    }
    if (_seenOpenBrace && _braceDepth <= 0)
    {
        _funcActive = false;
        _outParams.clear();
        _paramInjections.clear();
...
bool LineRewriter::ppActive() const
{
    for (const PPFrame& f : _ppStack)
        if (!f.active)
            return false;
    return true;
}

void LineRewriter::updatePreprocessor(const string& t)
{
    auto directive = [&](const char* d)
    {
        return t.compare(0, std::strlen(d), d) == 0;
    };
    if (directive("#ifdef") || directive("#ifndef") || directive("#if"))
    {
        const bool parentActive = ppActive();
        bool cond;
        if (directive("#ifdef"))
        ...
        const bool active = parentActive && cond;
        _ppStack.push_back({ active, active, parentActive });
    }
    else if (directive("#elif"))
    {
        if (!_ppStack.empty())
        {
            PPFrame& f = _ppStack.back();
...
{
    // Minimal C preprocessor for `#if/#elif/#else/#endif`: evaluate conditionals
    // (WGSL has none) and drop lines in inactive branches.
    {
        const string t = trim(line);
        if (!t.empty() && t[0] == '#')
        {
            if (t.compare(0, 3, "#if") == 0 || t.compare(0, 5, "#elif") == 0 ||
                t.compare(0, 5, "#else") == 0 || t.compare(0, 6, "#endif") == 0)
            {
                updatePreprocessor(t);
                return "";
            }
        }
        if (!ppActive())
            return ""; // inside an inactive preprocessor branch
    }

    // Struct definition body: rewrite GLSL members to WGSL and drop the trailing
    // semicolon on the closing brace.
    if (_inStruct)
    {
        const string t = trim(line);
        if (t == "};" || t == "}")
        {
            _inStruct = false;
            const size_t indentEnd = line.find_first_not_of(" \t");
            return (indentEnd == string::npos ? "" : line.substr(0, indentEnd)) + "}";
        }
        if (t.empty() || t[0] == '/')
            return line; // blank or comment
        return rewriteStructMember(line);
    }
    {
        const string t = trim(line);
        ...
        {
            _inStruct = true;
            return line; // `struct Name {` / `struct Name` is valid WGSL as-is
        }
    }
    ...
    if (_funcActive)
    {
        ...
        if (_pendingHeader)
        {
            ...
            if (bt.empty() || bt.compare(0, 2, "//") == 0 || bt.compare(0, 2, "/*") == 0)
                return "";
            _pendingHeader = false;
            string hdr = rewriteNonSignatureLine(derefOutParams(_headerLine, _outParams));
            string body = rewriteNonSignatureLine(derefOutParams(line, _outParams));
            countBraces(line, _braceDepth, _seenOpenBrace);
            if (_seenOpenBrace && _braceDepth <= 0)
            {
                _funcActive = false;
                _outParams.clear();
        ...
        // Accumulate a ternary expression that spans multiple lines (the per-line
        // rewrite needs the whole `cond ? a : b` on one line). A continuation begins
        // when a line ends with `?` and runs until the statement terminator `;`.
        if (_accumTernary)
        {
            _ternaryBuffer += " " + trim(line);
            if (!trim(line).empty() && trim(line).back() == ';')
            {
                const string joined = _ternaryBuffer;
                _accumTernary = false;
                _ternaryBuffer.clear();
                string done = derefOutParams(joined, _outParams);
                done = rewriteNonSignatureLine(std::move(done));
                countBraces(joined, _braceDepth, _seenOpenBrace);
                if (_seenOpenBrace && _braceDepth <= 0)
                {
                    _funcActive = false;
                    _outParams.clear();
        ...
        // Inside a function body: dereference out parameters, then apply the
        // shared syntax rewrites (body lines never match the signature heuristic).
        string out = derefOutParams(line, _outParams);
        out = rewriteNonSignatureLine(std::move(out));

        const bool wasOpen = _seenOpenBrace;
        countBraces(line, _braceDepth, _seenOpenBrace);
        if (!_injected && !wasOpen && _seenOpenBrace)
        {
            // The body brace just opened on this line: inject the value-param copies.
            for (const string& inj : _paramInjections)
                out += "\n" + inj;
            _injected = true;
        }
        if (_seenOpenBrace && _braceDepth <= 0)
    ...
    // Accumulating a signature whose parameter list spans multiple lines.
    if (_accumulatingSig)
    {
        _sigBuffer += " " + line;
        _sigParenDepth += netParens(line);
        if (_sigParenDepth > 0)
            return ""; // still open: defer emission until the list closes
        const string joined = _sigBuffer;
        _accumulatingSig = false;
        _sigBuffer.clear();
        std::vector<string> outNames;
        std::vector<std::pair<string, string>> valueParams;
        string sig;
        if (rewriteSignature(joined, sig, outNames, valueParams))
            return beginFunction(sig, std::move(outNames), valueParams);
        return rewriteNonSignatureLine(joined); // not a signature after all
    }
    ...
    // Single-line signature.
    {
        ...
        std::vector<std::pair<string, string>> valueParams;
        string sig;
        if (rewriteSignature(line, sig, outNames, valueParams))
            return beginFunction(sig, std::move(outNames), valueParams);
    }
...
// ─── rewriteLine / rewriteBlock ─────────────────────────────────────────────

string rewriteLine(const string& line)
{
    return LineRewriter().rewrite(line);
}
...
// Parse a `fn NAME(PARAMS)` definition line. Returns false if not a definition.
bool parseFnDef(const string& line, string& name, string& params)
{
    const size_t indentEnd = line.find_first_not_of(" \t");
    if (indentEnd == string::npos || line.compare(indentEnd, 3, "fn ") != 0)
        return false;
    ...
    if (close == string::npos)
        return false;
    name = trim(line.substr(nameStart, paren - nameStart));
                    ...
                    const std::map<string, std::map<string, string>>& structs)
{
    ...
    static const char* CTORS[] = { "vec2f", "vec3f", "vec4f", "mat2x2f", "mat3x3f", "mat4x4f", "f32", "i32", "u32" };
    for (const char* c : CTORS)
        if (a.compare(0, std::strlen(c), c) == 0 && a.size() > std::strlen(c) && a[std::strlen(c)] == '(')
            return c;
    if (a.compare(0, 5, "vec3<") == 0)
        return "vec3f";
    if (a.compare(0, 5, "vec2<") == 0)
        return "vec2f";
    if (a.compare(0, 5, "vec4<") == 0)
        return "vec4f";
...
void collectVarDecl(const string& line, std::map<string, string>& out)
{
    for (const char* kw : { "var ", "let ", "const " })
    {
        ...
        while ((k = line.find(kwStr, k)) != string::npos)
        {
            ...
            size_t ts = c + 1;
            while (ts < line.size() && isSpace(line[ts]))
                ts++;
            size_t te = ts;
            int angle = 0;
            while (te < line.size())
            {
                ...
                if (ch == '<')
                    ...
                    angle--;
                else if (angle == 0 && (ch == ';' || ch == '=' || isSpace(ch)))
                    break;
                te++;
            }
            if (te > ts && !name.empty())
...
string derefPointerParams(const string& shader)
{
    ...
    string line;
    auto braceDelta = [](const string& s)
    {
        int d = 0;
        for (char c : s)
        {
    ...
    while (std::getline(in, line))
    {
        ...
        {
            string name, params;
            if (parseFnDef(line, name, params))
            {
                inFn = true;
...
string dedupDefinitions(const string& shader)
{
    ...
    while (std::getline(in, line))
    {
        ...
        if (depth == 0 && !t.empty())
        {
            ...
            else if (t.compare(0, 3, "fn ") == 0)
            {
                string name, params;
                if (parseFnDef(line, name, params))
                {
                    string key = name + "(";
                    for (const string& ty : paramTypes(params))
                        key += ty + ",";
...
string coerceBoolCallSites(const string& shader)
{
    ...
    while (std::getline(pre, line))
    {
        ...
        {
            braceDepth = 0;
            const std::vector<string> types = paramTypes(params);
            ...
            if (anyBool)
                fnBool[name].push_back(boolFlags);
        }
...
string resolveOverloads(const string& shader)
{
    ...
    while (std::getline(pre, line))
    {
        ...
        if (parseFnDef(line, name, params))
        {
            braceDepth = 0;
            const std::vector<string> types = paramTypes(params);
            defs[name].push_back({ types, string() });
            ...
            if (arrow != string::npos)
            {
                ...
                ret = trim(line.substr(arrow + 2, (br == string::npos ? line.size() : br) - (arrow + 2)));
            }
            defRet[name].push_back(ret);
        }
        else if (braceDepth == 0)
        {
            collectVarDecl(line, globals);
    ...
    while (std::getline(in, line))
    {
        ...
        string name, params;
        const bool isDef = parseFnDef(line, name, params);
        if (isDef)
        {
            ...
            if (ov != overloads.end())
            {
                ...
                for (const auto& variant : ov->second)
                    ...
                    {
                        const size_t p = line.find("fn " + name + "(");
                        if (p != string::npos)
                            line.replace(p + 3, name.size(), variant.second);
                        break;
                    }
            }
        }
...
string repairEmptyElseCommentBlocks(const string& shader)
{
    ...
    for (size_t i = 0; i < lines.size(); ++i)
    {
        ...
        if (t.compare(0, 6, "else {") == 0 && t.find("//") != string::npos)
        {
...
StringVec findResidualGlsl(const string& wgsl)
{
    ...
    static const std::pair<const char*, const char*> markers[] = {
    ...
    {
        if (wgsl.find(marker.first) != string::npos)
        {
            findings.push_back(string("residual GLSL token '") + marker.first + "' (" + marker.second + ")");
        }
    }
    return findings;
}
