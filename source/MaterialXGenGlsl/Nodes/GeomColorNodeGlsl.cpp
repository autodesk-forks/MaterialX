...
void GeomColorNodeGlsl::createVariables(const ShaderNode& node, GenContext&, Shader& shader) const
{
    ...
    addStageInput(HW::VERTEX_INPUTS, Type::COLOR4, HW::IN_COLOR + "_" + index, vs);
    addStageConnector(HW::VERTEX_DATA, Type::COLOR4, HW::COLOR + "_" + index, vs, ps);
}
...
void GeomColorNodeGlsl::emitFunctionCall(const ShaderNode& node, GenContext& context, ShaderStage& stage) const
{
    ...
    string variable = HW::COLOR + "_" + index;
    ...
    BEGIN_SHADER_STAGE(stage, Stage::VERTEX)
        ...
        {
            color->setEmitted();
            shadergen.emitLine(prefix + color->getVariable() + " = " + HW::IN_COLOR + "_" + index, stage);
        }
