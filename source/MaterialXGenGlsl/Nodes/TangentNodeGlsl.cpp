...
void TangentNodeGlsl::createVariables(const ShaderNode& node, GenContext&, Shader& shader) const
{
    ...
    addStageInput(HW::VERTEX_INPUTS, Type::VECTOR3, HW::IN_TANGENT, vs);
    ...
    if (space == WORLD_SPACE)
    {
        ...
        addStageConnector(HW::VERTEX_DATA, Type::VECTOR3, HW::TANGENT_WORLD, vs, ps);
    }
    else
    {
        addStageConnector(HW::VERTEX_DATA, Type::VECTOR3, HW::TANGENT_OBJECT, vs, ps);
    }
}
...
void TangentNodeGlsl::emitFunctionCall(const ShaderNode& node, GenContext& context, ShaderStage& stage) const
{
    ...
    BEGIN_SHADER_STAGE(stage, Stage::VERTEX)
        ...
        if (space == WORLD_SPACE)
        {
            ShaderPort* tangent = vertexData[HW::TANGENT_WORLD];
            if (!tangent->isEmitted())
            {
                tangent->setEmitted();
                shadergen.emitLine(prefix + tangent->getVariable() + " = (" + HW::WORLD_INVERSE_TRANSPOSE_MATRIX + " * vec4(" + HW::IN_TANGENT + ",0.0)).xyz", stage);
            }
        }
        else
        {
            ShaderPort* tangent = vertexData[HW::TANGENT_OBJECT];
            if (!tangent->isEmitted())
            {
                tangent->setEmitted();
                shadergen.emitLine(prefix + tangent->getVariable() + " = " + HW::IN_TANGENT, stage);
            }
        }
    ...
    BEGIN_SHADER_STAGE(stage, Stage::PIXEL)
        ...
        if (space == WORLD_SPACE)
        {
            const ShaderPort* tangent = vertexData[HW::TANGENT_WORLD];
            shadergen.emitString(" = normalize(" + prefix + tangent->getVariable() + ")", stage);
        }
        else
        {
            const ShaderPort* tangent = vertexData[HW::TANGENT_OBJECT];
            shadergen.emitString(" = normalize(" + prefix + tangent->getVariable() + ")", stage);
        }
