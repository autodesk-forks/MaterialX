...
void BitangentNodeGlsl::createVariables(const ShaderNode& node, GenContext&, Shader& shader) const
{
    ...
    addStageInput(HW::VERTEX_INPUTS, Type::VECTOR3, HW::IN_BITANGENT, vs);
    ...
    if (space == WORLD_SPACE)
    {
        ...
        addStageConnector(HW::VERTEX_DATA, Type::VECTOR3, HW::BITANGENT_WORLD, vs, ps);
    }
    else
    {
        addStageConnector(HW::VERTEX_DATA, Type::VECTOR3, HW::BITANGENT_OBJECT, vs, ps);
    }
}
...
void BitangentNodeGlsl::emitFunctionCall(const ShaderNode& node, GenContext& context, ShaderStage& stage) const
{
    ...
    BEGIN_SHADER_STAGE(stage, Stage::VERTEX)
        ...
        if (space == WORLD_SPACE)
        {
            ShaderPort* bitangent = vertexData[HW::BITANGENT_WORLD];
            if (!bitangent->isEmitted())
            {
                bitangent->setEmitted();
                shadergen.emitLine(prefix + bitangent->getVariable() + " = (" + HW::WORLD_INVERSE_TRANSPOSE_MATRIX + " * vec4(" + HW::IN_BITANGENT + ",0.0)).xyz", stage);
            }
        }
        else
        {
            ShaderPort* bitangent = vertexData[HW::BITANGENT_OBJECT];
            if (!bitangent->isEmitted())
            {
                bitangent->setEmitted();
                shadergen.emitLine(prefix + bitangent->getVariable() + " = " + HW::IN_BITANGENT, stage);
            }
        }
    ...
    BEGIN_SHADER_STAGE(stage, Stage::PIXEL)
        ...
        if (space == WORLD_SPACE)
        {
            const ShaderPort* bitangent = vertexData[HW::BITANGENT_WORLD];
            shadergen.emitString(" = normalize(" + prefix + bitangent->getVariable() + ")", stage);
        }
        else
        {
            const ShaderPort* bitangent = vertexData[HW::BITANGENT_OBJECT];
            shadergen.emitString(" = normalize(" + prefix + bitangent->getVariable() + ")", stage);
        }
