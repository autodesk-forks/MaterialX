...
#ifndef MATERIALX_VKMATERIAL_H
#define MATERIALX_VKMATERIAL_H

/// @file
/// Vulkan material helper classes
...
using VkMaterialPtr = std::shared_ptr<class VkMaterial>;

/// @class VkMaterial
/// Helper class for Vulkan (GLSL→SPIR-V) generation and rendering of a material.
/// Mirrors GlslMaterial's ShaderMaterial override set.
class MX_RENDERVK_API VkMaterial : public ShaderMaterial
{
  public:
    VkMaterial() :
        ShaderMaterial()
    ...
    static VkMaterialPtr create()
    {
        return std::make_shared<VkMaterial>();
    }
    ...
    /// Generate a shader from our currently stored element and the given context.
    ...
    {
        _hwShader = std::static_pointer_cast<VkMaterial>(material)->_hwShader;
        _vkProgram = std::static_pointer_cast<VkMaterial>(material)->_vkProgram;
    }

    /// Return the underlying Vulkan program.
    VkProgramPtr getProgram() const { return _vkProgram; }

    /// Bind shader. A validation / lazy-build step: ensures _vkProgram->hasBuiltData(),
    /// builds if not. The command buffer is set by the renderer before the pass.
    ...
    /// Unbind all images for this material.
    ...
    /// Draw the given mesh partition. The command buffer must be set on the
    /// program before calling this (the renderer does this in its render pass).
