...
//
class VkShaderRenderTester : public RenderUtil::ShaderRenderTester
{
  public:
    explicit VkShaderRenderTester(mx::ShaderGeneratorPtr shaderGenerator) :
...
// document.
void VkShaderRenderTester::loadAdditionalLibraries(mx::DocumentPtr document,
...
void VkShaderRenderTester::registerLights(mx::DocumentPtr /*document*/,
...
//
void VkShaderRenderTester::createRenderer(std::ostream& log)
{
    bool initialized = false;
    try
    {
        _renderer = mx::VkRenderer::create(768,768);
...
bool VkShaderRenderTester::saveImage(const mx::FilePath& filePath, mx::ConstImagePtr image, bool verticalFlip) const
{
    return _renderer->getImageHandler()->saveImage(filePath, image, verticalFlip);
}

bool VkShaderRenderTester::runRenderer(const std::string& /*shaderName*/,
                                          mx::TypedElementPtr /*element*/,
                                          mx::GenContext& /*context*/,
                                          mx::DocumentPtr /*doc*/,
                                          std::ostream& /*log*/,
                                          ...
                                          mx::ImageVec* /*imageVec*/)
{
    ...
    #if 0
    std::cout << "Validating Vulkan GLSL rendering for: " << doc->getSourceUri() << std::endl;
    ...
    if (element && doc)
    {
        log << "------------ Run Vulkan GLSL validation with element: " << element->getNamePath() << "-------------------" << std::endl;
        ...
        for (auto options : optionsList)
        {
            ...
            if (testOptions.dumpGeneratedCode)
            {
                ...
                std::ofstream file;
                file.open(shaderPath + "_vk_vs.glsl");
                file << vertexSourceCode;
                file.close();
                file.open(shaderPath + "_vk_ps.glsl");
            ...
            MaterialX::VkProgramPtr program = _renderer->getProgram();
            ...
            try
            {
                ...
                if (!geomHandler->hasGeometry(geomPath))
                {
                    ...
                    {
                        vkaddAdditionalTestStreams(mesh);
                    }
                }
                ...
                if (testOptions.renderImages)
                {
                    ...
                    if (testOptions.saveImages)
                    {
                        ...
                        std::string fileName = shaderPath + "_vk_glsl.png";
            ...
            catch (mx::ExceptionRenderError& e)
            {
                ...
                log << ">> Refer to shader code in dump files: " << shaderPath << "_vk_ps.glsl and _vk_vs.glsl files" << std::endl;
...
TEST_CASE("Render: Vulkan TestSuite", "[rendervk]")
{
    mx::FileSearchPath searchPath = mx::getDefaultDataSearchPath();
    mx::FilePath optionsFilePath = searchPath.find("resources/Materials/TestSuite/_options.mtlx");

    VkShaderRenderTester renderTester(mx::VkShaderGenerator::create());
