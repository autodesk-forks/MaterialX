...
def usage():
    ...
    print 'Usage:  mxvalidate.py <file.mtlx> [<outputfile.md]'
...
def main():
    ...
    for nd in nodedefs:
        ...
        file.write('- *Type*: %s\n' % nd.getType())
        file.write('- *Doc*: %s\n' % nd.getAttribute('doc'))
        file.write('| Name | Type | Default Value | UI min | UI max | UI group | Doc | Advanced | Connectable |\n')
        file.write('| ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- |\n')
        ...
        for p in nd.getParameters():
            ...
            adv = inp.getAttribute('uiadvanced')
            buf = '| %s | %s | %s | %s | %s | %s | %s | %s | %s |\n' % (name, uimin, uimax, uifolder, docattr, adv, "false")
            file.write(buf)
