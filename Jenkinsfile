                    ...
                    stage("Create Nuget Packages") {
                        ...
                        withEnv(properties) {
                            if(axisNodeValue.contains("GEC-vs")) {
                                ...
                                nuget pack adsk-build-scripts/nuget/osx/adsk_materialx-sdk_osx_intel64.nuspec -Version $NUGET_VERSION -OutputDirectory $WORKSPACE/packages -Prop installdir=$WORKSPACE/install -Prop materialxcontrib=$WORKSPACE/source/MaterialXContrib -Prop materialx=${materialx_version} -Prop osx_compiler=${osx_compiler} -Prop osx_target=${osx_target}
                                """
                                ...
                                nuget pack adsk-build-scripts/nuget/linux/adsk_materialx-lib_linux_debug_intel64.nuspec -Version $NUGET_VERSION -OutputDirectory $WORKSPACE/packages -Prop installdir=$WORKSPACE/_build/install -Prop materialx=${materialx_version} -Prop linux_compiler=${linux_compiler} -Prop linux_target=${linux_target}
                                nuget pack adsk-build-scripts/nuget/linux/adsk_materialx-lib_linux_release_intel64.nuspec -Version $NUGET_VERSION -OutputDirectory $WORKSPACE/packages -Prop installdir=$WORKSPACE/_build/install -Prop materialx=${materialx_version} -Prop linux_compiler=${linux_compiler} -Prop linux_target=${linux_target}
