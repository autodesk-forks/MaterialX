                ...
                ws("${WorkDirComp}"){
                    ...
                    stage("Build") {
                        ...
                        if (axisNodeValue.contains("GEC-vs")) {
                            ...
                            sh "git clean -fdx"
                            sh '''
                            cmake -S . -G "Unix Makefiles" -B_build -DCMAKE_BUILD_TYPE=debug -DCMAKE_INSTALL_PREFIX=_build/install -DMATERIALX_BUILD_PYTHON=OFF  -DMATERIALX_BUILD_RENDER=ON -DMATERIALX_BUILD_TESTS=OFF -DCMAKE_DEBUG_POSTFIX=d  -DMATERIALX_BUILD_GEN_OSL=ON -DMATERIALX_BUILD_GEN_MDL=OFF -DMATERIALX_CONTRIB=ON -DMATERIALX_BUILD_VIEWER=OFF -DMATERIALX_BUILD_SHARED_LIBS=ON -DMATERIALX_INSTALL_INCLUDE_PATH=inc -DMATERIALX_INSTALL_LIB_PATH=libs -DMATERIALX_INSTALL_STDLIB_PATH=libraries 
                            cmake --build _build --config debug --target install --parallel 16
                            cmake -S . -G "Unix Makefiles" -B_build -DCMAKE_BUILD_TYPE=release -DCMAKE_INSTALL_PREFIX=_build/install -DMATERIALX_BUILD_PYTHON=OFF  -DMATERIALX_BUILD_RENDER=ON -DMATERIALX_BUILD_TESTS=OFF -DMATERIALX_BUILD_GEN_OSL=ON -DMATERIALX_BUILD_GEN_MDL=OFF -DMATERIALX_CONTRIB=ON -DMATERIALX_BUILD_VIEWER=OFF -DMATERIALX_BUILD_SHARED_LIBS=ON -DMATERIALX_INSTALL_INCLUDE_PATH=inc -DMATERIALX_INSTALL_LIB_PATH=libs -DMATERIALX_INSTALL_STDLIB_PATH=libraries
                    ...
                    stage("Create Nuget Packages") {
                        ...
                        withEnv(properties) {
                            if(axisNodeValue.contains("GEC-vs")) {
                                ...
                                nuget pack adsk-build-scripts\\nuget\\win\\adsk_materialx-content.nuspec -Version %NUGET_VERSION% -OutputDirectory %WORKSPACE%\\packages -Prop installdir=%WORKSPACE%\\install -Prop materialx=${materialx_version} -Prop materialxcontrib=%WORKSPACE%\\source\\MaterialXContrib -Prop win_compiler=${win_compiler}
                                nuget pack adsk-build-scripts\\nuget\\win\\adsk_materialx-sdk_win_intel64.nuspec -Version %NUGET_VERSION% -OutputDirectory %WORKSPACE%\\packages -Prop installdir=%WORKSPACE%\\install -Prop materialx=${materialx_version} -Prop materialxcontrib=%WORKSPACE%\\source\\MaterialXContrib -Prop win_compiler=${win_compiler}
                                nuget pack adsk-build-scripts\\nuget\\win\\adsk_materialx_win_debug_intel64.nuspec -Version %NUGET_VERSION% -OutputDirectory %WORKSPACE%\\packages -Prop installdir=%WORKSPACE%\\install -Prop materialx=${materialx_version} -Prop materialxcontrib=%WORKSPACE%\\source\\MaterialXContrib -Prop win_compiler=${win_compiler}
                                nuget pack adsk-build-scripts\\nuget\\win\\adsk_materialx_win_release_intel64.nuspec -Version %NUGET_VERSION% -OutputDirectory %WORKSPACE%\\packages -Prop installdir=%WORKSPACE%\\install -Prop materialx=${materialx_version} -Prop materialxcontrib=%WORKSPACE%\\source\\MaterialXContrib -Prop win_compiler=${win_compiler}
                                """
                                ...
                                nuget pack adsk-build-scripts/nuget/osx/adsk_materialx_osx_debug_ubx64arm64.nuspec -Version $NUGET_VERSION -OutputDirectory $WORKSPACE/packages -Prop installdir=$WORKSPACE/install -Prop materialx=${materialx_version} -Prop osx_compiler=${osx_compiler} -Prop osx_target=${osx_target}
                                nuget pack adsk-build-scripts/nuget/osx/adsk_materialx_osx_release_ubx64arm64.nuspec -Version $NUGET_VERSION -OutputDirectory $WORKSPACE/packages -Prop installdir=$WORKSPACE/install -Prop materialx=${materialx_version} -Prop osx_compiler=${osx_compiler} -Prop osx_target=${osx_target}
                                nuget pack adsk-build-scripts/nuget/osx/adsk_materialx-content.nuspec -Version $NUGET_VERSION -OutputDirectory $WORKSPACE/packages -Prop installdir=$WORKSPACE/install -Prop materialxcontrib=$WORKSPACE/source/MaterialXContrib -Prop materialx=${materialx_version} -Prop osx_compiler=${osx_compiler} -Prop osx_target=${osx_target}
                                nuget pack adsk-build-scripts/nuget/osx/adsk_materialx-sdk_osx_ubx64arm64.nuspec -Version $NUGET_VERSION -OutputDirectory $WORKSPACE/packages -Prop installdir=$WORKSPACE/install -Prop materialxcontrib=$WORKSPACE/source/MaterialXContrib -Prop materialx=${materialx_version} -Prop osx_compiler=${osx_compiler} -Prop osx_target=${osx_target}
                                """
                                zip zipFile: "${env.WORKSPACE}/packages/adsk_materialx-sdk_osx_ubx64arm64.${env.NUGET_VERSION}.zip", dir: "${env.WORKSPACE}/install", glob: '**/*'
                            } else {
                                sh """
                                nuget pack adsk-build-scripts/nuget/linux/adsk_materialx-headers_linux.nuspec -Version $NUGET_VERSION -OutputDirectory $WORKSPACE/packages -Prop installdir=$WORKSPACE/_build/install -Prop materialx=${materialx_version} -Prop linux_compiler=${linux_compiler} -Prop linux_target=${linux_target}
                                nuget pack adsk-build-scripts/nuget/linux/adsk_materialx_linux_debug_intel64.nuspec -Version $NUGET_VERSION -OutputDirectory $WORKSPACE/packages -Prop installdir=$WORKSPACE/_build/install -Prop materialx=${materialx_version} -Prop linux_compiler=${linux_compiler} -Prop linux_target=${linux_target}
                                nuget pack adsk-build-scripts/nuget/linux/adsk_materialx_linux_release_intel64.nuspec -Version $NUGET_VERSION -OutputDirectory $WORKSPACE/packages -Prop installdir=$WORKSPACE/_build/install -Prop materialx=${materialx_version} -Prop linux_compiler=${linux_compiler} -Prop linux_target=${linux_target}
                                nuget pack adsk-build-scripts/nuget/linux/adsk_materialx-content.nuspec -Version $NUGET_VERSION -OutputDirectory $WORKSPACE/packages -Prop installdir=$WORKSPACE/_build/install -Prop materialxcontrib=$WORKSPACE/source/MaterialXContrib -Prop materialx=${materialx_version} -Prop linux_compiler=${linux_compiler} -Prop linux_target=${linux_target}
                                nuget pack adsk-build-scripts/nuget/linux/adsk_materialx-sdk_linux_intel64.nuspec -Version $NUGET_VERSION -OutputDirectory $WORKSPACE/packages -Prop installdir=$WORKSPACE/_build/install -Prop materialxcontrib=$WORKSPACE/source/MaterialXContrib -Prop materialx=${materialx_version} -Prop linux_compiler=${linux_compiler} -Prop linux_target=${linux_target}
                                """
                                zip zipFile: "${env.WORKSPACE}/packages/adsk_materialx-sdk_linux_intel64.${env.NUGET_VERSION}.zip", dir: "${env.WORKSPACE}/_build/install", glob: '**/*'
                            }
                        }
                    }
