...
void adsk_NG_adsk_metal(vec3 in2, vec3 specular_color, float roughness, float anisotropy, float rotation, vec3 normal, vec3 opacity, out surfaceshader out1)
{
    vec3 geomprop_Nworld_out1 = normalize(vd.normalWorld);
    vec3 geomprop_Tworld_out1 = normalize(vd.tangentWorld);
    vec3 base_color_value_out = specular_color * in2;
...
void main()
{
    vec3 geomprop_Nworld_out1 = normalize(vd.normalWorld);
    surfaceshader surfaceshader_out = surfaceshader(vec3(0.0),vec3(0.0));
    adsk_NG_adsk_metal(metal_f0, surface_albedo, surface_roughness, surface_anisotropy, surface_rotation, geomprop_Nworld_out1, surface_cutout, surfaceshader_out);
