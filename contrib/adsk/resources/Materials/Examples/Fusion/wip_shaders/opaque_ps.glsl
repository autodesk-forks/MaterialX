...
void adsk_NG_adsk_opaque(vec3 normal, vec3 opacity, vec3 specular_color, vec3 base_color, float roughness, float anisotropy, float rotation, float f0, bool value1, vec3 subsurface_radius, float subsurface_scale, bool value11, float in1, vec3 emission_color, out surfaceshader out1)
{
    float specular_roughness_value_out = 0.0;
    adsk_NG_adsk_roughness_adjust(roughness, anisotropy, specular_roughness_value_out);
    float specular_IOR_value_out = 0.0;
    adsk_NG_adsk_f0_to_ior(f0, specular_IOR_value_out);
    float specular_anisotropy_value_out = 0.0;
    adsk_NG_adsk_anisotropy_adjust(anisotropy, specular_anisotropy_value_out);
    float specular_rotation_value_out = 0.0;
    adsk_NG_adsk_rotation_normalize(rotation, specular_rotation_value_out);
    const bool subsurface_value_value2_tmp = true;
    const float subsurface_value_in1_tmp = 1.000000;
    const float subsurface_value_in2_tmp = 0.000000;
    float subsurface_value_out = (value1 == subsurface_value_value2_tmp) ? subsurface_value_in1_tmp : subsurface_value_in2_tmp;
    vec3 geomprop_Nworld_out1 = normalize(vd.normalWorld);
    const bool emission_value_value2_tmp = true;
    const float emission_value_in2_tmp = 0.000000;
    float emission_value_out = (value11 == emission_value_value2_tmp) ? in1 : emission_value_in2_tmp;
    vec3 geomprop_Tworld_out1 = normalize(vd.tangentWorld);
    const float emission_value_scaled_in1_tmp = 0.001953;
    float emission_value_scaled_out = emission_value_scaled_in1_tmp * emission_value_out;
    surfaceshader surfaceshader_out = surfaceshader(vec3(0.0),vec3(0.0));
    NG_standard_surface_surfaceshader_optim(1.000000, base_color, 0.000000, 0.000000, 1.000000, specular_color, specular_roughness_value_out, specular_IOR_value_out, specular_anisotropy_value_out, specular_rotation_value_out, 0.000000, vec3(1.000000, 1.000000, 1.000000), 0.000000, vec3(0.000000, 0.000000, 0.000000), 0.000000, 0.000000, 0.000000, subsurface_value_out, base_color, subsurface_radius, subsurface_scale, 0.000000, 0.000000, vec3(1.000000, 1.000000, 1.000000), 0.300000, 0.000000, vec3(1.000000, 1.000000, 1.000000), 0.100000, 0.000000, 0.000000, 1.500000, geomprop_Nworld_out1, 0.000000, 0.000000, 0.000000, 1.500000, emission_value_scaled_out, emission_color, opacity, false, normal, geomprop_Tworld_out1, surfaceshader_out);
    out1 = surfaceshader_out;
}
...
void main()
{
    vec3 geomprop_Nworld_out1 = normalize(vd.normalWorld);
    surfaceshader surfaceshader_out = surfaceshader(vec3(0.0),vec3(0.0));
    adsk_NG_adsk_opaque(geomprop_Nworld_out1, surface_cutout, surface_albedo, opaque_albedo, surface_roughness, surface_anisotropy, surface_rotation, opaque_f0, opaque_translucency, opaque_mfp_modifier, opaque_mfp, opaque_emission, opaque_luminance, opaque_luminance_modifier, surfaceshader_out);
