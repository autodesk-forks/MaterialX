...
void NG_standard_surface_surfaceshader_optim(float in2, vec3 in1, float roughness, float weight, float in21, vec3 in11, float bg, float ior, float anisotropy, float in12, float weight1, vec3 tint, float transmission_depth, vec3 transmission_scatter, float transmission_scatter_anisotropy, float transmission_dispersion, float in22, float weight2, vec3 in13, vec3 in14, float in23, float anisotropy1, float sheen, vec3 sheen_color, float sheen_roughness, float weight3, vec3 fg, float roughness1, float anisotropy2, float in15, float ior1, vec3 normal, float in24, float in16, float thinfilm_thickness, float thinfilm_ior, float in25, vec3 in17, vec3 in18, bool in19, vec3 normal1, vec3 in26, out surfaceshader out1)
{
    vec2 coat_roughness_vector_out = vec2(0.0);
    mx_roughness_anisotropy(roughness1, anisotropy2, coat_roughness_vector_out);
    ...
    float coat_affect_roughness_multiply1_out = in16 * weight3;
    ...
    float coat_clamped_out = clamp(weight3, coat_clamped_low_tmp, coat_clamped_high_tmp);
    ...
    vec3 coat_attenuation_out = mix(coat_attenuation_bg_tmp, fg, weight3);
    ...
    surfaceshader surfaceshader_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        for (int activeLightIndex = 0; activeLightIndex < numLights; ++activeLightIndex)
        {
            ...
            mx_dielectric_bsdf(closureData, weight3, vec3(1.000000, 1.000000, 1.000000), ior1, coat_roughness_vector_out, 0.000000, 1.500000, normal, coat_tangent_out, 0, 0, coat_bsdf_out);
        ...
        // Add surface emission
        {
            ...
            mx_mix_edf(closureData, coat_emission_edf_out, emission_edf_out, weight3, blended_coat_emission_edf_out);
