...
layout (std140, binding=1) uniform PrivateUniforms_pixel
{
    ...
    int u_refractionTwoSided;
#define u_refractionTwoSided bool(u_refractionTwoSided)
    vec3 u_viewPosition;
    int u_numActiveLightSources;
};
layout (binding=2) uniform texture2D u_envRadiance_texture;
layout (binding=3) uniform sampler u_envRadiance_sampler;
layout (binding=4) uniform texture2D u_envIrradiance_texture;
layout (binding=5) uniform sampler u_envIrradiance_sampler;

// Uniform block: PublicUniforms
layout (std140, binding=6) uniform PublicUniforms_pixel
{
    ...
    float surface_anisotropy;
    float surface_rotation;
    vec3 metal_f0;
    vec3 opaque_albedo;
    float opaque_f0;
    int opaque_translucency;
#define opaque_translucency bool(opaque_translucency)
    vec3 opaque_mfp_modifier;
    float opaque_mfp;
    int opaque_emission;
#define opaque_emission bool(opaque_emission)
    float opaque_luminance;
    vec3 opaque_luminance_modifier;
    float transparent_ior;
    vec3 transparent_color;
    float transparent_distance;
    float layered_f0;
    float layered_roughness;
    float layered_anisotropy;
    float layered_rotation;
    float layered_fraction;
    int layered_fraction_invert;
#define layered_fraction_invert bool(layered_fraction_invert)
    vec3 layered_diffuse;
    vec3 layered_bottom_f0;
    vec3 glazing_transmission_color;
    float glazing_transmission_roughness;
    vec3 glazing_f0;
    int glazing_backface_culling;
#define glazing_backface_culling bool(glazing_backface_culling)
};
...
vec3 mx_latlong_map_lookup(vec3 dir, mat4 transform, float lod, texture2D tex_texture, sampler tex_sampler)
{
    vec3 envDir = normalize(mx_matrix_mul(transform, vec4(dir,0.0)).xyz);
    vec2 uv = mx_latlong_projection(envDir);
    return textureLod(sampler2D(tex_texture, tex_sampler), uv, lod).rgb;
}
...
vec3 mx_environment_radiance(vec3 N, vec3 V, vec3 X, vec2 alpha, int distribution, FresnelData fd)
{
    ...
    for (int i = 0; i < envRadianceSamples; i++)
    {
        ...
        vec3 sampleColor = mx_latlong_map_lookup(Lw, u_envMatrix, lod, u_envRadiance_texture, u_envRadiance_sampler);
...
{
    vec3 Li = mx_latlong_map_lookup(N, u_envMatrix, 0.0, u_envIrradiance_texture, u_envIrradiance_sampler);
...
layout (std140, binding=7) uniform LightData_pixel
{
...
void NG_standard_surface_surfaceshader_optim(float in2, vec3 in1, float roughness, float weight, float in21, vec3 in11, float bg, float ior, float anisotropy, float in12, float in13, vec3 transmission_color, float transmission_depth, vec3 transmission_scatter, float transmission_scatter_anisotropy, float transmission_dispersion, float in22, float in14, vec3 in15, vec3 in16, float in23, float subsurface_anisotropy, float sheen, vec3 sheen_color, float sheen_roughness, float weight1, vec3 fg, float roughness1, float anisotropy1, float in17, float ior1, vec3 normal, float in24, float in18, float thinfilm_thickness, float thinfilm_ior, float in25, vec3 in19, vec3 in110, bool in111, vec3 normal1, vec3 in26, out surfaceshader out1)
{
    vec2 coat_roughness_vector_out = vec2(0.0);
    mx_roughness_anisotropy(roughness1, anisotropy1, coat_roughness_vector_out);
    const float coat_tangent_rotate_degree_in2_tmp = 360.000000;
    float coat_tangent_rotate_degree_out = in17 * coat_tangent_rotate_degree_in2_tmp;
    vec3 metal_reflectivity_out = in1 * in2;
    vec3 metal_edgecolor_out = in11 * in21;
    float coat_affect_roughness_multiply1_out = in18 * weight1;
    ...
    vec3 subsurface_color_nonnegative_out = max(in15, subsurface_color_nonnegative_in2_tmp);
    const float coat_clamped_low_tmp = 0.000000;
    const float coat_clamped_high_tmp = 1.000000;
    float coat_clamped_out = clamp(weight1, coat_clamped_low_tmp, coat_clamped_high_tmp);
    vec3 subsurface_radius_scaled_out = in16 * in23;
    float subsurface_selector_out = float(in111);
    ...
    float subsurface_inv_out = subsurface_inv_amount_tmp - in14;
    const float transmission_inv_amount_tmp = 1.000000;
    float transmission_inv_out = transmission_inv_amount_tmp - in13;
    ...
    vec3 coat_attenuation_out = mix(coat_attenuation_bg_tmp, fg, weight1);
    ...
    vec3 emission_weight_out = in19 * in25;
    vec3 opacity_luminance_out = vec3(0.0);
    mx_luminance_color3(in110, vec3(0.272229, 0.674082, 0.053689), opacity_luminance_out);
    ...
    float coat_affect_roughness_multiply2_out = coat_affect_roughness_multiply1_out * roughness1;
    ...
    vec3 coat_tangent_out = (anisotropy1 > coat_tangent_value2_tmp) ? coat_tangent_rotate_normalize_out : in26;
    ...
    surfaceshader surfaceshader_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        for (int activeLightIndex = 0; activeLightIndex < numLights; ++activeLightIndex)
        {
            ...
            mx_dielectric_bsdf(closureData, weight1, vec3(1.000000, 1.000000, 1.000000), ior1, coat_roughness_vector_out, 0.000000, 1.500000, normal, coat_tangent_out, 0, 0, coat_bsdf_out);
            ...
            mx_mix_bsdf(closureData, BSDF(vec3(0.0),vec3(1.0)), BSDF(vec3(0.0),vec3(1.0)), subsurface_selector_out, selected_subsurface_bsdf_out);
            ...
            mx_layer_bsdf(closureData, BSDF(vec3(0.0),vec3(1.0)), subsurface_blend_out, sheen_layer_out);
            ...
            mx_add_bsdf(closureData, BSDF(vec3(0.0),vec3(1.0)), sheen_layer_non_transmission_out, transmission_blend_out);
            ...
            surfaceshader_out.color += lightShader.intensity * coat_layer_out.response;
        ...
        // Add environment contribution
        {
            ...
            surfaceshader_out.color += occlusion * coat_layer_out.response;
        }
        ...
        // Add surface emission
        {
            ...
            mx_mix_edf(closureData, coat_emission_edf_out, emission_edf_out, weight1, blended_coat_emission_edf_out);
            surfaceshader_out.color += blended_coat_emission_edf_out;
        }
        ...
        BSDF coat_layer_out = BSDF(vec3(0.0),vec3(1.0));
        mx_layer_bsdf(closureData, coat_bsdf_out, thin_film_layer_attenuated_out, coat_layer_out);
        surfaceshader_out.color += coat_layer_out.response;

        // Compute and apply surface opacity
        {
            surfaceshader_out.color *= surfaceOpacity;
            surfaceshader_out.transparency = mix(vec3(1.000000, 1.000000, 1.000000), surfaceshader_out.transparency, surfaceOpacity);
        }
    }

    out1 = surfaceshader_out;
}
